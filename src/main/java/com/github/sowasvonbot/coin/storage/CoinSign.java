...
public class CoinSign {
  ...
  private static final NamespacedKey OWNER =
      NamespacedKey.fromString("coin_sign_owner", CoinsPlugin.COINS_PLUGIN);
  private static final NamespacedKey KEY =
      NamespacedKey.fromString("coin_sign", CoinsPlugin.COINS_PLUGIN);
  ...
  private static void addCoinChestToPlayer(Player player, Location location) {
    ...
    if (newChest.isEmpty()) {
      player.sendMessage(ConfigHolder.getInstance()
          .getString(ConfigHolder.ConfigField.ERROR_CREATE_COIN_CHEST));
